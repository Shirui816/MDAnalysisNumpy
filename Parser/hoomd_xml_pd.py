...
from DataStructure.DataframeNames import dtypeDict

try:
    from pandas import read_csv as loadtxt
except ImportError:
    warnings.warn("No module pandas, I can't accelerate while files are large.")
    ...
    def __init__(self, filename, needed=[]):
        ...
        for e in configuration:
            ...
            try:
                # print(e.tag)
                dt = dtypeDict[e.tag]
                self.nodes[e.tag] = loadtxt(StringIO(e.text), delim_whitespace=True, names=dt).values
            except KeyError:
                self.nodes[e.tag] = loadtxt(StringIO(e.text), delim_whitespace=True).values
